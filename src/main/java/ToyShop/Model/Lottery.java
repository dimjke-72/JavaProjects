...
public class Lottery extends Mode {
    ...
    public void execute(ToysDistributor toys, Scanner scanner) {
        if (toys.getToys() == null) {
            ...
            boolean game = true;
            do {
                System.out.println(name + " для начала розыгрыша нажмите enter");
                scanner.next();
                ...
                if (count == 0){
                    game = false;
                    System.out.println("всё разыграли");
                }
            } while (game);
     ...
     * @return id игрушки, -1 не возвращается
     */
